...
class ReplayBuffer:
    ...
    def addTransition(
            ...
            rewards:np.ndarray, costs:np.ndarray, dones:np.ndarray, fails:np.ndarray, next_states:np.ndarray) -> None:
    ...
    def getBatches(
        ...
        reward_critic:torch.nn.Module, cost_critics:list,
        ...
        for env_idx in range(self.n_envs):
            ...
            # get means and stds
            means_tensor, log_stds_tensor, stds_tensor = actor(states_tensor)
            next_means_tensor, next_log_stds_tensor, next_stds_tensor = actor(next_states_tensor)
            ...
            # get targets
            next_reward_quantiles_tensor = reward_critic(
                next_states_tensor, next_means_tensor, next_log_stds_tensor).reshape(actions_tensor.shape[0], -1)
            ...
            for cost_idx in range(len(cost_critics)):
                next_cost_quantiles_tensor = cost_critics[cost_idx](
                    next_states_tensor, next_means_tensor, next_log_stds_tensor).reshape(actions_tensor.shape[0], -1)
            ...
            reward_values = reward_critic(states_tensor, means_tensor, log_stds_tensor).reshape(
                                actions_tensor.shape[0], -1).mean(dim=-1).detach().cpu().numpy()
            reward_delta = 0.0
        ...
        for cost_idx in range(len(cost_critics)):
            cost_targets_tensor_list.append(torch.tensor(np.concatenate(cost_targets_lists[cost_idx], axis=0), device=self.device, dtype=torch.float32))
