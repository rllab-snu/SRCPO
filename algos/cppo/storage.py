...
class ReplayBuffer:
    ...
    def addTransition(
            ...
            next_states:np.ndarray, discounted_cost_sums:np.ndarray, discounts:np.ndarray) -> None:
        for env_idx in range(self.n_envs):
            self.storage[env_idx].append([
                ...
                discounted_cost_sums[env_idx], discounts[env_idx],
            ])

    @torch.no_grad()
    def getBatches(
        self, obs_rms, reward_rms, actor:torch.nn.Module, 
        reward_critic:torch.nn.Module
        ...
        for env_idx in range(self.n_envs):
            ...
            cost_returns = np.zeros_like(discounted_cost_sums)
            for t in reversed(range(len(reward_targets))):
                ...
                if t == len(reward_targets) - 1:
                    cost_returns[t, :] = discounted_cost_sums[t]
                else:
                    if dones[t]:
                        cost_returns[t, :] = discounted_cost_sums[t]
                    else:
                        cost_returns[t, :] = cost_returns[t+1, :]
            ...
            cost_returns_list.append(cost_returns)
        ...
        cost_returns_tensor = torch.tensor(np.concatenate(cost_returns_list, axis=0), device=self.device, dtype=torch.float32)
